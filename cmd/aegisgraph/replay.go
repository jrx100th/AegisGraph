...
func makeReplayScenario(name string) ReplayScenario {
	...
	switch name {
	case "public-ssh","internet-privileged-role","internet-sensitive-s3","internet-denied-sensitive-s3","assume-role-chain","role-chain-sensitive","role-cycle-sensitive","multiple-security-groups","out-of-order-pages":
		makePublicSSH(c)
